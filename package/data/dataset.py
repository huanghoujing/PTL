...
class Dataset(TorchDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.has_attr_label and cfg.use_attr_label:
    ...
    # This is for pseudo attr
    def get_attr_label(self, im_path):
        if not hasattr(self, 'pseudo_attr'):
            self.pseudo_attr = load_pickle(osp.join(self.cfg.root, 'predicted_attr/{}_to_{}/{}_attr.pkl'.format(self.cfg.attr_format, self.cfg.name, self.cfg.pseudo_attr_soft_or_hard)), verbose=False)
        if self.cfg.pseudo_attr_soft_or_hard == 'soft':
            attr_label = [attr[im_path] for attr in self.pseudo_attr]
        elif self.cfg.pseudo_attr_soft_or_hard == 'hard':
            attr_label = self.pseudo_attr[im_path]
        else:
            raise ValueError('Invalid pseudo_attr_soft_or_hard: {}'.format(self.cfg.pseudo_attr_soft_or_hard))
        return attr_label
